    ...
    try:
        ...
        if stats_response.status_code == 200:
            ...
            if not eval_readiness.get('documents_indexed', False):
                ...
                if st.button("📄 Add Sample Content for Evaluation"):
                    with st.spinner("Adding evaluation-ready sample content..."):
                        try:
                            response = requests.post(f"{API_BASE_URL}/add-sample-content", timeout=30)
                            if response.status_code == 200:
                                st.success("✅ Sample content added! Ready for evaluation.")
...
def upload_and_qa():
    ...
    st.write("Try the enhanced sample content with evaluation capabilities!")
    
    if st.button("📄 Add Enhanced Sample Content"):
        with st.spinner("Adding enhanced sample content..."):
            try:
                response = requests.post(f"{API_BASE_URL}/add-sample-content", timeout=30)
...
def test_documents():
    """Enhanced document testing interface with evaluation readiness"""
    st.header("🧪 Document Processing Test & Evaluation Readiness")
    
    st.markdown("""
    ### Document Testing & Evaluation Readiness
    
    Test your documents for:
    - **Processing Quality**: How well text can be extracted
    - **Evaluation Readiness**: Whether the document is suitable for SQUAD/COQA evaluation
    - **Performance Impact**: How the document affects system performance
    - **Quality Assessment**: Overall document quality scoring
    """)
    ...
    test_file = st.file_uploader(
        "Upload a document to test processing quality and evaluation readiness",
        type=['pdf', 'docx', 'txt', 'html', 'md'],
        help="This will analyze your document without adding it to the Q&A system"
    )
    
    if test_file:
        if st.button("🔍 Analyze Document", type="primary"):
            with st.spinner("Analyzing document for evaluation readiness..."):
                try:
                    files_data = [("file", (test_file.name, test_file.getvalue(), test_file.type))]
                    response = requests.post(f"{API_BASE_URL}/test-document", files=files_data, timeout=60)
                    ...
                    if response.status_code == 200:
                        ...
                        eval_ready = result.get('evaluation_ready', False)
                        if eval_ready:
                            st.success("🎯 **Document is ready for evaluation!**")
                        else:
                            st.warning("⚠️ **Document has limited evaluation readiness**")
                        ...
                        with col1:
                            ...
                            st.metric("Text Quality", f"{text_quality:.2f}")
                        
                        with col2:
                            chunk_quality = quality.get('chunk_quality', 0)
                            st.metric("Chunk Quality", f"{chunk_quality:.2f}")
                        
                        with col3:
                            embedding_quality = quality.get('embedding_quality', 0)
                            st.metric("Embedding Quality", f"{embedding_quality:.2f}")
                        
                        with col4:
                            overall_score = quality.get('overall_score', 0)
                            st.metric("Overall Score", f"{overall_score:.2f}")
                        
                        # Text analysis with readability
                        ...
                        with col2:
                            st.metric("Paragraphs", text_analysis.get('paragraph_count', 0))
                        with col3:
                            readability = text_analysis.get('readability_score', 0)
                            st.metric("Readability", f"{readability:.3f}")
                        with col4:
                            st.metric("Characters", text_analysis.get('total_characters', 0))
                        ...
                        with col2:
                            avg_chunk_size = chunk_analysis.get('average_chunk_size', 0)
                            st.metric("Avg Chunk Size", f"{avg_chunk_size:.0f}")
                        with col3:
                            optimal = chunk_analysis.get('optimal_chunk_count', False)
                            st.metric("Optimal Chunks", "✅ Yes" if optimal else "❌ No")
                        
                        # Embedding test results
                        embedding_test = result.get('embedding_test')
                        if embedding_test:
                            st.subheader("🔗 Embedding Generation Test")
                            if embedding_test.get('success'):
                                st.success(f"✅ Embeddings generated successfully ({embedding_test.get('embedding_dimensions', 0)} dimensions)")
                            else:
                                st.error(f"❌ Embedding generation failed: {embedding_test.get('error', 'Unknown error')}")
                        ...
                        st.subheader("🎯 Evaluation Suitability")
                        
                        word_count = text_analysis.get('word_count', 0)
                        chunk_count = chunk_analysis.get('total_chunks', 0)
                        
                        eval_col1, eval_col2 = st.columns(2)
                        
                        with eval_col1:
                            st.write("**SQUAD Evaluation Suitability:**")
                            if word_count > 500 and chunk_count >= 3:
                                st.success("✅ Excellent for SQUAD evaluation")
                            elif word_count > 200 and chunk_count >= 2:
                                st.warning("⚠️ Suitable for basic SQUAD evaluation")
                            else:
                                st.error("❌ Not suitable for SQUAD evaluation")
                        
                        with eval_col2:
                            st.write("**Performance Impact:**")
                            if chunk_count <= 10:
                                st.success("✅ Low performance impact")
                            elif chunk_count <= 20:
                                st.warning("⚠️ Moderate performance impact")
                            else:
                                st.error("❌ High performance impact")
